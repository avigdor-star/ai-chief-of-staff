...
### Replaced captains-log skill with log skill (documentation router)
- **Files:** new `skills/log/SKILL.md`; deleted `skills/captains-log/`; edited `skills/chief-of-staff/SKILL.md`, `skills/chief-of-staff/references/captains-log.md`, `README.md`, `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`. Version bumped 0.3.0 → 0.3.1.
- **What:** The standalone `captains-log` skill is gone. In its place is a new `log` skill — a lightweight documentation router. It triggers on all quick-documentation phrases ("log this", "document this", "record this", "note this down", "journal this", "journal that", "remember this", "capture this", "captain's log", "captains log", "captain's log: …"), parses the content, runs a targeted keyword search against the Tasks database and State Dashboard, and routes to the right destination — task update, project note, dashboard update, or (for reflective/personal content) a new Captain's Log entry. The Chief of Staff skill's trigger list was updated to REMOVE generic documentation phrases (those now go to `log`). The Chief's internal documentation routing block was reframed: it still runs when the Chief is already active inside a session, but it no longer triggers on cold-start "log this" messages — the `log` skill handles those. The Captain's Log database, schema, and `references/captains-log.md` are unchanged and still shared between both skills.
- **Why:** The old architecture had a routing hole. The `captains-log` skill would auto-write to the Captain's Log even when the content belonged in a task or on the State Dashboard — and it couldn't hand off to the Chief of Staff (skills can't invoke each other). A real incident exposed it: a car mileage update was dropped into the Captain's Log despite having a matching open task and a watch list item. The earlier Step 2.5 patch only flagged the mismatch and told the user to manually invoke the Chief, which defeated the point of a quick-log skill. The new `log` skill has the routing brain built in — it writes to operational records directly, so misfires don't bypass the routing logic anymore.
